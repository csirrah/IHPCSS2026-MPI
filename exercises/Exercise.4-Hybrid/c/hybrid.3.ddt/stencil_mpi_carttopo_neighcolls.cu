...
int main(int argc, char **argv) {
  ...
  /* ===================================================================== */
  // Step 1. Create datatypes for row and column
  // row datatype
  MPI_Datatype row_type;
  MPI_Type_contiguous(/* TODO: count, oldtype, *newtype */);
  MPI_Type_commit(/* TODO: *newtype */);
  // column datatype
  int pitch = bx+2;
  MPI_Datatype column_type;
  MPI_Type_vector(/* TODO: count, blocklength, stride, oldtype, *new_type */);
  MPI_Type_commit(/* TODO: *new_type */);
  /* ===================================================================== */

  /* ===================================================================== */
  // Step 2. Prepare arrays to describe Neighbor_alltoallw (note byte offsets)
  int send_counts[4] = {1,1,1,1};
  int recv_counts[4] = {1,1,1,1};
  MPI_Aint send_offs[4];
  send_offs[0] = ind(/* TODO,TODO */) * sizeof(double);  // inner west
  send_offs[1] = ind(/* TODO,TODO */) * sizeof(double); // inner east
  send_offs[2] = ind(/* TODO,TODO */) * sizeof(double);  // inner north
  send_offs[3] = ind(/* TODO,TODO */) * sizeof(double); // inner south
  MPI_Aint recv_offs[4];
  recv_offs[0] = ind(/* TODO,TODO */) * sizeof(double);    // outer west
  recv_offs[1] = ind(/* TODO,TODO */) * sizeof(double); // outer east
  recv_offs[2] = ind(/* TODO,TODO */) * sizeof(double);    // outer north
  recv_offs[3] = ind(/* TODO,TODO */) * sizeof(double); // outer south
  MPI_Datatype send_types[4];
  send_types[0] = /* TODO */; // west exchange
  send_types[1] = /* TODO */; // east exchange
  send_types[2] = /* TODO */; // north exchange
  send_types[3] = /* TODO */; // south exchange
  MPI_Datatype recv_types[4];
  recv_types[0] = /* TODO */; // west exchange
  recv_types[1] = /* TODO */; // east exchange
  recv_types[2] = /* TODO */; // north exchange
  recv_types[3] = /* TODO */; // south exchange
  /* ===================================================================== */ 
  ...
  // GPU: allocate and zero sbuf and rbuf on device
  ...
  //size_t bbytes = 2*bx*sizeof(double)+2*by*sizeof(double);
  //double *sbuf;
  //cudaMalloc(&sbuf, bbytes);
  //cudaMemset(sbuf, 0, bbytes);
  //double *rbuf;
  //cudaMalloc(&rbuf, bbytes);
  //cudaMemset(rbuf, 0, bbytes);
  ...
  for(int iter=0; iter<niters; ++iter) {
    ...
    /* ===================================================================== */
	// Step 3: call MPI_Neighbor_alltoallw
    MPI_Request req;
    MPI_Status status;
    MPI_Ineighbor_alltoallw(/* TODO: *sendbuff, sendcounts, senddispls, sendtypes, *recvbuff, recvcounts, recvdipls, recvtypes, comm, *request */);
    MPI_Wait(&req, &status);
    /* ===================================================================== */
  ...
  // GPU-aware: these are now device allocations
  //cudaFree(sbuf);
  //cudaFree(rbuf);

  // GPU ddt: free datatypes
