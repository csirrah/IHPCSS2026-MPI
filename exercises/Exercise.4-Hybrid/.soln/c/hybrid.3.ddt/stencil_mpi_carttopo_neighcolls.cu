...
int main(int argc, char **argv) {
  ...
  // GPU ddt: row datatype
  MPI_Datatype row_type;
  MPI_Type_contiguous(bx,MPI_DOUBLE,&row_type);
  MPI_Type_commit(&row_type);

  // GPU ddt: column datatype
  int pitch = bx+2;
  MPI_Datatype column_type;
  MPI_Type_vector(by,1,pitch,MPI_DOUBLE,&column_type);
  MPI_Type_commit(&column_type);

  // GPU ddt: arrays for Neighbor_alltoallw (note byte offsets)
  int send_counts[4] = {1,1,1,1};
  int recv_counts[4] = {1,1,1,1};
  MPI_Aint send_offs[4];
  send_offs[0] = ind(1,1) * sizeof(double);  // inner west
  send_offs[1] = ind(bx,1) * sizeof(double); // inner east
  send_offs[2] = ind(1,1) * sizeof(double);  // inner north
  send_offs[3] = ind(1,by) * sizeof(double); // inner south
  MPI_Aint recv_offs[4];
  recv_offs[0] = ind(0,1) * sizeof(double);    // outer west
  recv_offs[1] = ind(bx+1,1) * sizeof(double); // outer east
  recv_offs[2] = ind(1,0) * sizeof(double);    // outer north
  recv_offs[3] = ind(1,by+1) * sizeof(double); // outer south
  MPI_Datatype send_types[4];
  send_types[0] = column_type; // west exchange
  send_types[1] = column_type; // east exchange
  send_types[2] = row_type; // north exchange
  send_types[3] = row_type; // south exchange
  MPI_Datatype recv_types[4];
  recv_types[0] = column_type; // west exchange
  recv_types[1] = column_type; // east exchange
  recv_types[2] = row_type; // north exchange
  recv_types[3] = row_type; // south exchange
  ...
  for(int iter=0; iter<niters; ++iter) {
	...
	// GPU ddt: use MPI_Neighbor_alltoallw
    MPI_Request req;
    MPI_Status status;
    MPI_Ineighbor_alltoallw(d_aold, send_counts, send_offs, send_types, d_aold, recv_counts, recv_offs, recv_types, topocomm, &req);
    MPI_Wait(&req, &status);
